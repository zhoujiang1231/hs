...
<link type="text/css" rel="stylesheet" media="all" href="${pageContext.request.contextPath}/css/global_color.css" />
	        ...
	        function seareach(){
        		...
        		$.ajax({
                    ...
                    success:function (data) {
                        $.each(data.list,function (student) {
                            $("#table_tr").append("<tr><td>"+student.stuNo+"</td><td>"+student.stuAddress+"</td><td>"+student.stuSex+"</td>" +
                                "<td>"+student.college.college+"</td><td>"+student.college.professional+"</td><td>"+student.college.s_class+"</td><td>"+student.stuIdcard+"</td><td>"+student.stuEmail+"</td><td>"+student.stuTel+"</td><td>"+student.stuNo+"</td></tr>");
                        })
                    }
                })
	        }
	      ...
	      function showPage(cur){
           ...
           function dele(id){
            if(window.confirm("确定要删除此账务账号吗(删除操作不可撤销)?")){
            	$.ajax({
            		url:'${pageContext.request.contextPath }/student/deleteUser',
            		type:'post',
            		data:stuId=id,
            		success:function(data){
            			if(data==1){
            ...
            function details(id){
            	$("#hidden").attr("value",id);
            	$("#hidden").attr("stuAddress","details");
            	$("#form").submit();
            }
    ...
    <body stuAddress="account_off">
        <!--主要区域开始-->
        <div id="main1">
                <div class="search_add">
                    <div>
                        学院：
                        <select id="college" stuAddress="college" class="select_search">
                            <option value="10">全部</option>
                        </select>
                    </div>
                    <div>
                        专业：
                        <select id="professional" stuAddress="professional" class="select_search">
                            <option value="10">全部</option>
                        </select>
                    </div>
                    <div>
                        班级：
                        <select id="s_class" stuAddress="s_class" class="select_search">
                            <option value="10">全部</option>
                    ...
                    <div>
                        性别：
                        <select id="stuSex" stuAddress="stuSex" class="select_search">
                            <option value="10">全部</option>
                ...
                <div id="data">            
                 ...
                 <input type="hidden" stuAddress="" id="hidden" value=""/>
